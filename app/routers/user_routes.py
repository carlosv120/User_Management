...
@router.get("/users/nickname/{nickname}", response_model=UserResponse, name="get_user_by_nickname", tags=["User Management Requires (Admin or Manager Roles)"])
async def get_user_by_nickname(nickname: str, request: Request, db: AsyncSession = Depends(get_db), token: str = Depends(oauth2_scheme), current_user: dict = Depends(require_role(["ADMIN", "MANAGER"]))):
    """
